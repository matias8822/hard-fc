...
export default function App() {
  ...
  // ------- estilos globales ligeros (hover / animaciones) -------
  const PlayerStyles = () => (
    <style>{`
      .player {
        /* Sin animaciones */
      ...
      .player:hover {
        ...
        box-shadow: 0 10px 18px rgba(0,0,0,.5), inset 0 0 22px rgba(255,255,255,.4);
      }
      .player--dragging {
        transform: translate(-50%, -50%) scale(0.98);
        box-shadow: 0 6px 14px rgba(0,0,0,.45), inset 0 0 16px rgba(255,255,255,.35);
      }
      .btn {
        padding: 6px 10px;
        border-radius: 10px;
    ...
    >
      ...
      >
        ...
        >
          ...
          <button onClick={resetPositions} className="btn btn--warn" title="Volver al layout de la formación">
        ...
        >
          ...
          />
          <button onClick={savePreset} className="btn" style={{ background: "linear-gradient(180deg,#22c55e,#16a34a)", borderColor:"rgba(34,197,94,.3)", color:"#04120a" }}>
            Guardar
          </button>
          ...
          <button onClick={exportPNG} className="btn btn--neutral" title="Exportar PNG">PNG</button>
        </div>
      </div>
